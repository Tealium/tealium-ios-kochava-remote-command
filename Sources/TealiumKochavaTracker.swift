...
public class TealiumKochavaTracker: NSObject, KochavaTrackable, TealiumRegistration, TealiumDeepLinkable {
    ...
    public func configure(with parameters: [AnyHashable: Any]) {
        ...
        if let sleep = parameters[KochavaConstants.Keys.sleepTracker] as? Bool,
            sleep == true {
            sleepTracker(sleep)
        }
    }
    ...
    public func sendEvent(name: String) {
        KochavaTracker.shared.sendEvent(withNameString: name, infoDictionary: nil)
    }
    ...
    public func sendEvent(type: KochavaEventTypeEnum) {
        if let event = KochavaEvent(eventTypeEnum: type) {
            KochavaTracker.shared.send(event)
        }
    }
    
    public func sendEvent(type: KochavaEventTypeEnum, with dictionary: [String : Any]) {
         if let event = KochavaEvent(eventTypeEnum: type) {
    ...
    public func application(_ application: TealiumApplication,
