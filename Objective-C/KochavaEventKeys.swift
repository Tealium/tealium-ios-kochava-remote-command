...
public class KochavaEventKeys: NSObject, Codable {
    ...
    @objc public class func create(from dictionary: [String: Any]) -> KochavaEventKeys? {
        let decoder = JSONDecoder()
        do {
            let jsonData = try JSONSerialization.data(withJSONObject: dictionary, options:[])
            let eventKeys = try decoder.decode(KochavaEventKeys.self, from: jsonData)
            return eventKeys
        } catch {
            print("\(KochavaConstants.errorPrefix) could not decode event keys.")
        }
        return nil
    }
